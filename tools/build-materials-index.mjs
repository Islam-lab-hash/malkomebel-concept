...
const cards = materialSources.map((source, i) => {
  ...
  return `<a class="source-catalog-card" href="./${match.file}"><img loading="lazy" src="./${image}" alt="${esc(source.headings?.[0] || source.title)}"><div><strong>${esc(source.headings?.[0] || source.title)}</strong><small>${esc(source.title)}</small><p>${esc((source.paragraphs?.[0] || '').slice(0, 190))}</p></div></a>`;
}).join('');
