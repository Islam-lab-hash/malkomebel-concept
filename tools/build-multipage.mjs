...
const archivePage = `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Все страницы | Малко-Мебель</title><link rel="stylesheet" href="./styles.css"></head><body>${nav('./')}<main><section class="archive-hero wrap"><p class="eyebrow">/ карта исходного сайта</p><h1 class="display">Все страницы<br><em>в одном месте</em></h1><p class="lead-dark">${records.length} публичных записей маршрутов исходного сайта. Каждая сохранена отдельным URL в переработанном интерфейсе, с исходным заголовком, текстом и ссылкой на оригинал.</p><label class="archive-search"><span>Поиск по страницам</span><input id="archive-search" type="search" placeholder="Кухни, шкафы, материалы..."></label></section><section class="archive-summary wrap"><div><strong>${records.length}</strong><span>записей маршрутов</span></div><div><strong>${groups.size}</strong><span>разделов</span></div><div><strong>5 910</strong><span>исходных ассетов в архиве</span></div></section><section class="archive-content wrap" id="archive-content">${groupMarkup}</section></main>${footer('./')}<script src="./app.js"></script></body></html>`;
